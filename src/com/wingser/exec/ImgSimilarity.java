...
//		// 获取图像
//		 File imageFile1 = new
//		 File("F:\\workspace_acg\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\acgweb\\uploads\\task\\1496212755337.jpg");
//		 File file2 = new
//		 File("F:\\workspace_acg\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\acgweb\\uploads\\task\\1496212755337.jpg");
...
//	}
	...
	public static int getSimilarity(File imageFile1, File file2) throws IOException {
		int[] pixels1 = getImgFinger(imageFile1); 
		int[] pixels2 = getImgFinger(file2);  
		return getSimilarity(pixels1, pixels2);
	}
	
	
	public static int getSimilarity(int[] pixels1, int[] pixels2) throws IOException {
		// 获取两个图的汉明距离（假设另一个图也已经按上面步骤得到灰度比较数组）
		int hammingDistance = getHammingDistance(pixels1, pixels2); 
		// 通过汉明距离计算相似度，取值范围 [0.0, 1.0]  
		int similarity = (int)(calSimilarity(hammingDistance)*100); 
	...
	public static int[] getImgFinger(File imageFile) throws IOException {
		...
		// 获取灰度像素的比较数组（即图像指纹序列） 
		pixels1 = getPixelDeviateWeightsArray(pixels1, averageColor); return pixels1;
	...
	public static BufferedImage toGrayscale(Image image) {  
		...
		BufferedImage grayBuffered = op.filter(sourceBuffered, null); return grayBuffered;
